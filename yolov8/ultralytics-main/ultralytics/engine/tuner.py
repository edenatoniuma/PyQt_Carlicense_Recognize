...
Example:
    Tune hyperparameters for YOLOv8n on COCO8 at imgsz=640 and epoch=30 for 300 tuning iterations.
    ```python
    from ultralytics import YOLO

    model = YOLO('yolov8n.pt')
    model.tune(data='coco8.yaml', epoch=10, iterations=300, optimizer='AdamW', plots=False, save=False, val=False)
    ```
"""
import random
import shutil
...
class Tuner:
    ...
    def __init__(self, args=DEFAULT_CFG, _callbacks=None):
        ...
        self.space = {  # key: (min, max, gain(optional))
            ...
            'warmup_epochs': (0.0, 5.0),  # warmup epoch (fractions ok)
