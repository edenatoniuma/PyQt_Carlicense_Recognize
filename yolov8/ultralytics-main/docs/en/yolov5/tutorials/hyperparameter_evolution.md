...
# python train.py --batch 64 --cfg yolov5n6.yaml --weights '' --data coco.yaml --img 640 --epoch 300 --linear
...
warmup_epochs: 3.0  # warmup epoch (fractions ok)
...
```bash
python train.py --epoch 10 --data coco128.yaml --weights yolov5s.pt --cache
```
...
```bash
# Single-GPU
python train.py --epoch 10 --data coco128.yaml --weights yolov5s.pt --cache --evolve

# Multi-GPU
for i in 0 1 2 3 4 5 6 7; do
  sleep $(expr 30 \* $i) &&  # 30-second delay (optional)
  echo 'Starting GPU '$i'...' &&
  nohup python train.py --epoch 10 --data coco128.yaml --weights yolov5s.pt --cache --device $i --evolve > evolve_gpu_$i.log &
done
