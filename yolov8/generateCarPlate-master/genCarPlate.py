...
class GenPlateScene:
    ...
    def gen_batch(self, perSize, outDir):
        ...
        for i in range(perSize * 31 - 1):
            ...
            if img is None:
                continue
            cv2.imencode('.jpg', img)[1].tofile(os.path.join(outputPath, plate_str + ".jpg"))
            # cv2.imwrite(os.path.join(outputPath, plate_str + ".jpg"), img)
...
def parse_args():
    ...
    parser.add_argument('gen_count_per_province', default=100, type=int, help='gen car plt num for per_province')
